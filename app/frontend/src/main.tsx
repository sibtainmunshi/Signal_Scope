...
function App() {
  ...
  return <div className="shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-note"><div className="sidebar-signal" aria-hidden="true"><Fingerprint size={34} strokeWidth={1} /></div><span className="eyebrow">A LITTLE MORE PERSPECTIVE</span><strong>Evidence before<br />certainty.</strong><p>Inspect the signal.<br />Understand what it can tell you.</p></div>
    ...
    <div className="workspace">
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {page === 'report' && <>
          ...
          <div className="notice"><Info size={18} /><div><strong>Read the numbers in context.</strong><p>Self-evaluated public benchmarks. Organizer baseline and hidden-test scores were not supplied. Only results provided for the loaded checkpoint appear here.</p></div></div>
          ...
          <div className="section-title"><h2>In-domain validation</h2><span>CHECKPOINT-SPECIFIC METRICS</span></div>
          ...
          <div className="report-grid"><section className="panel report-panel"><div className="report-title"><Fingerprint size={18} /><h2>Reproducibility record</h2></div>{[['Model', model?.model_version], ['Architecture', model?.architecture], ['Training source', model?.training_source], ['Source resolution', model?.source_resolution], ['Crop input', model?.image_size ? `${model.image_size} × ${model.image_size} px` : undefined], ['Decision threshold', model?.threshold != null ? percentage(model.threshold) : undefined], ['Calibration', model?.calibrated != null ? model.calibrated ? 'Development-calibrated' : 'Uncalibrated' : undefined], ['Validation samples', model?.metrics?.count?.toLocaleString()], ['Unseen-generator result', model?.unseen_generator_status]].map(([key, value]) => <div className="metadata-row" key={key}><span>{key}</span><strong>{value || 'Not supplied'}</strong></div>)}<div className="checkpoint-hash"><span>CHECKPOINT SHA-256</span><code>{model?.checkpoint_sha256 || 'No checkpoint loaded'}</code></div></section>
          <section className="panel report-panel"><div className="report-title"><Activity size={18} /><h2>Validation confusion matrix</h2></div><p className="muted">Rows are actual labels; columns are predicted labels.</p>{model?.metrics?.confusion_matrix ? <div className="matrix"><div /><div>Predicted real</div><div>Predicted AI</div><div>Actual real</div>{model.metrics.confusion_matrix[0].map((number, index) => <strong key={index}>{number.toLocaleString()}</strong>)}<div>Actual AI</div>{model.metrics.confusion_matrix[1].map((number, index) => <strong key={index}>{number.toLocaleString()}</strong>)}</div> : <div className="report-empty"><FlaskConical size={30} strokeWidth={1.3} /><strong>No validation matrix supplied</strong><p>The model service has not supplied a matrix for this checkpoint.</p></div>}<div className="report-note"><Info size={15} /><p>ROC-AUC measures score ranking. Accuracy and false-positive rate depend on the chosen threshold. These describe a benchmark, not certainty for an individual image.</p></div></section></div>
        </>}
        {page === 'about' && <>
