// Record genuine local inference. Narration clips/timings are prepared in tmp/demo.
// Requires installed Chrome and Playwright FFmpeg; no prediction mocking or network model API.
...
const out = path.join(root, cliOnly ? 'tmp/demo/recording_cli_fix' : 'tmp/demo/recording');
...
async function caption(text, number) {
  await page.evaluate(({text, number, total}) => {
    ...
    overlay.style.cssText = 'position:fixed;z-index:10000;bottom:0;left:0;right:0;background:#102427;color:#fff;padding:17px 42px;box-shadow:0 -3px 20px #0002;font:25px/1.35 Arial;pointer-events:none';
    ...
    overlay.append(label, line); document.body.append(overlay); document.body.style.paddingBottom = '135px';
...
async function analyze(filename) {
  ...
  await page.getByLabel('Choose image', { exact: true }).setInputFiles(path.join(root, 'report/explanation_samples', filename));
  ...
  evidence.push({filename, prediction: result.prediction, explanation: result.explanation.statements,
    masking: result.explanation.masking_diagnostic, robustness: result.robustness});
...
const timings=[];
try {
 for (const segment of segments.filter(s => !cliOnly || s.id >= 12)) {
  ...
  switch(segment.action) {
   ...
   case 'generated': await analyze('generated_detected.png'); break;
   ...
   case 'masking': await page.locator('.tab-content').evaluate(el=>el.scrollIntoView({block:'center'})); break;
   case 'failure': await analyze('generated_missed.png'); break;
   ...
   case 'final': await page.getByRole('button',{name:'Model report',exact:true}).click(); await focusHeading('Reserved generators: final results'); break;
   case 'method': await focusHeading('External development'); break;
   case 'indomain': await focusHeading('CIFAKE author test'); break;
   case 'cli': {
    const stdout=execFileSync(path.join(root,'.venv/Scripts/python.exe'),['model/predict.py','--image','report/explanation_samples/generated_detected.png','--device','cpu'],{cwd:root,encoding:'utf8',env:{...process.env,SIGNALSCOPE_DEVICE:'cpu'}});
    const prediction=JSON.parse(stdout); evidence.push({actual_cli_output:prediction});
    await page.setContent(`<html><body style="margin:0;background:#f5f7f5;color:#102427;font:19px/1.4 Arial;padding:36px 55px"><h1>One checkpoint. App, API and CLI.</h1><div style="display:grid;grid-template-columns:1.3fr 1fr;gap:35px"><div><h2>Command executed during this recording</h2><pre style="font:15px/1.35 Consolas;white-space:pre-wrap;background:white;padding:20px">.venv/Scripts/python model/predict.py --image report/explanation_samples/generated_detected.png --device cpu\n\n${escape(stdout)}</pre></div><div><h2>Evaluator setup instructions</h2><pre style="font:16px/1.8 Consolas;white-space:pre-wrap;overflow-wrap:anywhere;background:white;padding:20px">git clone https://github.com/sibtainmunshi/Signal_Scope.git\ncd Signal_Scope\npython scripts/setup.py\npython scripts/run.py</pre><p>44.8 MB checkpoint, verified by SHA-256.<br>Python packages are additional first-setup downloads.<br>No training dataset or GPU is needed for inference.</p><h2>Check reported statistics</h2><pre style="font:18px Consolas">python scripts/verify_frozen_results.py</pre><p>Archived measured scores; no re-evaluation of final images.</p></div></div></body></html>`); break;
   }
   case 'ending': await page.setContent(`<html><body style="margin:0;background:#102427;color:white;font:24px/1.5 Arial;padding:65px 90px"><div style="font-size:15px;letter-spacing:4px;color:#a3c6b7">SIH 2026 / SIGNALSCOPE</div><h1 style="font-size:54px;margin-bottom:10px">Evidence before certainty.</h1><p>Actual local predictions. Reproducible code. Reported failures.</p><div style="display:flex;gap:65px;margin:45px 0"><div><b style="font-size:46px">0.565</b><br>Reserved AUC, as distributed</div><div><b style="font-size:46px">0.643</b><br>Reserved AUC, format-matched</div></div><p>Research prototype: most unseen AI images are missed at the fixed threshold.<br>Grad-CAM is attribution, not verified artifact detection.</p><p style="color:#bbd4ca;font-size:21px">github.com/sibtainmunshi/Signal_Scope<br>README / report/model_report.pdf / report/explanation_samples</p><p style="font-size:16px;color:#bbd4ca">Demo images: GenImage validation (Zhu et al., NeurIPS 2023), BigGAN / SD1.5.<br>CC BY-NC-SA 4.0 and noncommercial research terms. Source hashes are in the repository.<br>Computer narration: Windows Microsoft Zira. Human explanation review remains pending.</p></body></html>`); break;
  }
