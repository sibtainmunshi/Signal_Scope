...
CACHED = {"pil_bilinear_v1", "pil_center_crop_v1"}


def cached_scores(detector, domain, split, genimage_cache):
    """Score the stored 160 px cache; valid only where it equals inference preprocessing."""
    dataset = MixedImages(split, cifake_limit=0, domain=domain, genimage_root=genimage_cache)
    ...
    with torch.inference_mode():
        for images, y, locations in DataLoader(dataset, batch_size=64):
            ...
            indices.extend(int(data.indices[int(i)]) for i in locations)
    return indices, labels, scores


def raw_scores(detector, domain, split, manifest):
    """Score original GenImage files or stored CIFAKE pixels directly through the Detector."""
    if domain == "genimage":
        rows = sorted((r for r in manifest if r["split"] == split), key=lambda r: int(r["cache_index"]))
        indices, labels = [int(r["cache_index"]) for r in rows], [int(r["label"]) for r in rows]

        def load(position):
            with Image.open(ROOT / rows[position]["path"]) as image:
                return ImageOps.exif_transpose(image).convert("RGB")
    else:
        data = CifakeDataset(ROOT / "data/processed/cifake", split)
        indices = [int(i) for i in data.indices]
        labels = [int(data.labels[i]) for i in indices]

        def load(position):
            return Image.fromarray(np.array(data.images[indices[position]]))
    scores = []
    for start in range(0, len(indices), 32):
        scores.extend(detector.score_images([load(p) for p in range(start, min(start + 32, len(indices)))]))
    return indices, labels, scores
...
def main():
    ...
    native = detector.preprocessing == "native_multicrop_v1"
    if not native and (detector.preprocessing not in CACHED or detector.image_size != 160):
        raise ValueError("Unsupported preprocessing for this evaluator")
    ...
    for domain in ("genimage", "cifake"):
        if native:
            cache_indices, labels, scores = raw_scores(detector, domain, args.split, manifest)
        else:
            cache_indices, labels, scores = cached_scores(detector, domain, args.split, genimage_cache)
    ...
    if native:
        verification = {
            "method": "Original GenImage files and stored CIFAKE pixels scored directly by Detector.score_images; no training cache involved",
            "checkpoint_sha256": detector.checkpoint_hash,
        }
    else:
    ...
    print("Preprocessing verification:", verification, flush=True)
