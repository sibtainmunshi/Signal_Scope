...
- [x] **Found and fixed a tower-download 404 present since v0.3.0:** the manifest/release JSON referenced an invented asset filename instead of the real uploaded one (`visual_fp16.ts`); no fresh clone could ever have downloaded the tower before this fix. Fixed on `main` and verified (curl 200 + full fresh-clone run above). The `v0.4.0` git tag itself still points at the pre-fix commit — moving it needs a force-push, which was intentionally left for the user to authorize rather than done automatically.
