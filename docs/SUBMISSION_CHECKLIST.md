...
- [ ] **Updated 3-5 minute v0.4.0 demo (PS Section 7.4, "primary evidence"; scored directly under Presentation & Demo, 5 pts, and feeds User Experience, 10 pts)** showing actual prediction, explanation, robustness, and an honest failure case (e.g. a missed CommunityForensics-generator image). Existing 4m15s v0.2.0 demo is labelled historical and does not reflect this model or UI. Wait for the UI polish pass to finish first so the demo shows the final interface, not an intermediate one.
