...
<p>Measured CPU components (shared CLIP tower): predict <b>~0.38s/image</b>, explanation <b>~5.5s</b>, robustness <b>~5.1s</b>. Fresh-clone timing not yet re-measured for this release.</p>
</section></div>
...
<h2>Measured result: this release on its own untouched holdouts</h2>
<table><tr><th>Holdout (never used in training)</th><th>Accuracy</th><th>Real-&gt;real</th><th>AI-&gt;AI</th><th>AUC</th></tr>
<tr><td>AI Detect Arena (17 current generators, n=755)</td><td>{100*mlp["aida_holdout"]["accuracy"]:.1f}%</td><td>{100*(1-mlp["aida_holdout"]["false_positive_rate"]):.1f}%</td><td>{100*mlp["aida_holdout"]["true_positive_rate"]:.1f}%</td><td>{mlp["aida_holdout"]["roc_auc"]:.3f}</td></tr>
<tr><td>CommunityForensics-Eval (~20 generators, n=1374)</td><td>{100*mlp["communityforensics_holdout"]["accuracy"]:.1f}%</td><td>{100*(1-mlp["communityforensics_holdout"]["false_positive_rate"]):.1f}%</td><td>{100*mlp["communityforensics_holdout"]["true_positive_rate"]:.1f}%</td><td>{mlp["communityforensics_holdout"]["roc_auc"]:.3f}</td></tr>
</table>
<p class="small">Real-photo recognition is strong on both (92.7%/97.8%). AI recall is decent on AIDA (78.5%), weaker on CommunityForensics (54.4%, which mixes older GAN generators with current diffusion models). Aggregate accuracy looks better than AI-recall alone because real photos dominate correctness in both sets. CommunityForensics-Eval is CC BY-NC-SA 4.0 (Park et al., CVPR 2025).</p>
...
<h2>Limitations</h2>
<p><b>Explanation audit not re-run for this architecture:</b> the 17/40-localised, p=0.29/0.82 findings in prior reports are from v0.3.0's linear head. Input-gradient explanation works for the MLP head (verified live) but has not had its own 40-image audit. <b>Module G robustness</b> and the <b>private user-image veto check</b> have likewise not been re-run for v0.4.0 as of this report. No generator attribution, caption consistency or C2PA signature verification. No organizer hidden-test score is available or claimed.</p>
